...
| MP-004 | Performance Measurement And Hotspot Removal | Add measurement first, then remove the highest-value local bottlenecks. | The repo now has first-pass measurements for transcription cold starts, speaker-segment retranscription, summary calls, and summary-aware control-plane scans, so the next fixes should follow the validated costs rather than speculation. | The repo has a lightweight measurement path for the major hotspots, and the dominant local bottlenecks have a verified mitigation. | [DEVNOTES.md](../DEVNOTES.md), [tech-debt.md](tech-debt.md), [src/minutes/adapters/transcriber_sensevoice.py](../src/minutes/adapters/transcriber_sensevoice.py), [src/minutes/orchestrator.py](../src/minutes/orchestrator.py), [src/minutes/storage/file_store.py](../src/minutes/storage/file_store.py) |
...
6. Extend MP-005 incrementally only when new artifact types or freshness rules would otherwise duplicate retrieval logic again.
